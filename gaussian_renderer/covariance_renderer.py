...
def render(
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            shs_view = shs.transpose(1, 2).view(-1, 3, (max_sh_degree + 1) ** 2)
...
def get_max_alphas(
    ...
    retriever_settings = AlphaRetrievalSettings(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
    ...
    retriever = AlphaRetriever(retriever_settings=retriever_settings)
    ...
    # Apply Gaussian binary mask to scales and opacity (Compact 3D)
    if hasattr(pc, "_mask"):
        mask = (
    ...
    # Rasterize visible Gaussians to image, obtain their radii (on screen).
    gaussian_alphas, gaussian_counts, radii = retriever(
        means3D=means3D,
        means2D=means2D,
        opacities=opacity,
        scales=scales,
