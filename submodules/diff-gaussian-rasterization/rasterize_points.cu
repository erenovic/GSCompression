...
#include "cuda_rasterizer/alpha_retriever.h"
		...
		bool debug)
{
	...
	torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
	torch::Tensor out_alpha = torch::full({P, 1}, 0.0, float_opts);
	torch::Tensor gaussian_count = torch::full({P}, 0, float_opts);
	...
	if(P != 0)
	{
		...
		rendered = AlphaRetriever::Retriever::forward(
			geomFunc,
			binningFunc,
			imgFunc,
			P, degree, M,
			...
			gaussian_count.contiguous().data<float>(),
			radii.contiguous().data<int>(),
			debug);
	}
	return std::make_tuple(rendered, out_alpha, gaussian_count, radii, geomBuffer, binningBuffer, imgBuffer);
}