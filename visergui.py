...
if args.ckpt is None:
    ...
    means = ckpt[0][1]
    quats = F.normalize(ckpt[0][5], p=2, dim=-1)
    scales = torch.exp(ckpt[0][4])
    opacities = torch.sigmoid(ckpt[0][6])
    sh0 = ckpt[0][2]
    shN = ckpt[0][3]
    ...
    aabb_min = torch.min(means, dim=0)[0]
    aabb_max = torch.max(means, dim=0)[0]

    box_d = aabb_max - aabb_min
    box_min = (aabb_min - 0.01 * box_d).contiguous()
    box_max = (aabb_max + 0.01 * box_d).contiguous()

    aabb = torch.cat([box_min, box_max], dim=0).cuda()
    ...
    opacities = opacities[:, 0].repeat(repeats**2)
