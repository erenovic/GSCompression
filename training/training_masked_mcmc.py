...
from gaussian_renderer.masked_renderer import render
from models.compression.compression_utils import get_size_in_bits
from models.splatting.masked_mcmc_model import GaussianModel
...
def training(
    ...
    for iteration in range(first_iter, optimization.iterations + 1):
        ...
        # Gaussian binary mask loss (Compact 3D)
        mask_loss = torch.mean((torch.sigmoid(gaussians._mask)))

        loss = (
            (1.0 - optimization.lambda_dssim) * Ll1
            + optimization.lambda_dssim * (1.0 - ssim(image, gt_image))
            + optimization.lambda_mask * mask_loss
        )
        ...
        with torch.no_grad():
            ...
            if (
                ...
                dead_mask = torch.logical_or(
                    (torch.sigmoid(gaussians._mask) <= optimization.mask_prune_threshold).squeeze(),
                    (gaussians.get_opacity < 0.005).squeeze(),
                ).unsqueeze(-1)
                gaussians.relocate_gs(dead_mask=dead_mask)
                gaussians.add_new_gs(cap_max=dataset.cap_max)
            else:
                if iteration % optimization.mask_prune_iter == 0:
                    gaussians.gaussian_mask_prune(
                        mask_prune_threshold=optimization.mask_prune_threshold
                    )
