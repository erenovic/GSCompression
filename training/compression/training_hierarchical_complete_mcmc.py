...
def training(
    ...
    for iteration in range(first_iter, optimization.iterations + 1):
        ...
        out_res_scaling = res_scaling.clone()
        out_res_rotation = res_rotation.clone()
        out_res_shs = res_shs.clone()
        out_res_opacity = res_opacity.clone()

        out_res_pos[inter_indices] = inter_compressed_attrs[:, :3]
        out_res_scaling[inter_indices] = inter_compressed_attrs[:, 3:6]
        out_res_rotation[inter_indices] = inter_compressed_attrs[:, 6:10]
        ...
        loss: torch.Tensor = (
            ...
            + (
                ...
                + optimization.lambda_rec * scaling_loss
                + optimization.lambda_rec * rotation_loss
        ...
        with torch.no_grad():
            ...
            ema_loss_for_scaling = 0.4 * scaling_loss.item() + 0.6 * ema_loss_for_scaling
