...
class LatentRepresentations(torch.nn.Module):
    def __init__(self, gaussians):
        ...
        self.mlp_sh = torch.nn.Sequential(
            torch.nn.Linear(32, 32),
            torch.nn.ReLU(),
        ...
        self.mlp_scaling = torch.nn.Sequential(
            torch.nn.Linear(32, 32),
            torch.nn.ReLU(),
            torch.nn.Linear(32, 3),
        )
        self.mlp_rotation = torch.nn.Sequential(
            torch.nn.Linear(32, 32),
            torch.nn.ReLU(),
    ...
    def forward(self, indices):
        opacity = torch.sigmoid(self.mlp_opacity(self.latents[indices]))
        sh = self.mlp_sh(self.latents[indices]).reshape(-1, 16, 3)
    ...
    def optimize_latents(self, gaussians, scene, pipeline, bg):
        ...
        for _ in tqdm(range(1000)):
            ...
            loss = (
                ...
                + l1_loss(rotations, gaussians.get_rotation[indices])
                + l1_loss(shs, gaussians.get_features[indices])
            )
            loss.backward()
            self.optimizer.step()
...
def training(
    ...
    latents.optimize_latents(gaussians, scene, pipeline, background)
    ...
    for iteration in range(first_iter, optimization.iterations + 1):
        ...
        if iteration > COMPRESSION_TRAIN_ITER_FROM:
            ...
            indices = torch.arange(gaussians._xyz.shape[0], device="cuda")
            scales, rotations, shs, opacities = latents(indices)
        ...
        if iteration % 100 == 0:
            ...
            logger.save_image(
                ...
                f"{logger.rendering_dir}/{iteration}_{viewpoint_cam.image_name}.png",
            )
        ...
        with torch.no_grad():
            ...
            if (iteration in optimization.test_iterations) and (
                iteration > COMPRESSION_TRAIN_ITER_FROM + 5000
            ):
            ...
            if iteration < optimization.iterations:
                ...
                latents.optimizer.step()
                latents.optimizer.zero_grad(set_to_none=True)
