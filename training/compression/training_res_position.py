...
def training(
    ...
    if checkpoint:
        ...
        if dataset.freeze_geometry is not None:
            logger.info("Freezing geometry for compression")
    ...
    for iteration in range(first_iter, optimization.iterations + 1):
        ...
        if iteration > COMPRESSION_TRAIN_ITER_FROM:
            ...
            res_positions = gaussians.get_xyz[sorted_indices] - mean3D[sorted_node_assignments]

            gaussian_params, likelihoods, indices, num_coeffs, subsample_fraction = compressor(
                res_positions, gaussians, iteration, entropy_lambda_level
            )
            num_coeffs = len(indices) * num_coeffs

            positions = gaussians.get_xyz.clone()
            positions[indices] = mean3D[sorted_node_assignments][indices] + gaussian_params[0]
            opacities = gaussians.get_opacity.clone()
            opacities[indices] = gaussians.opacity_activation(gaussian_params[1])
            scales = gaussians.get_scaling.clone()
            scales[indices] = gaussians.scaling_activation(gaussian_params[2])
            rotations = gaussians.get_rotation.clone()
            rotations[indices] = gaussians.rotation_activation(gaussian_params[3])
            shs = gaussians.get_features.clone()
            shs[indices] = gaussian_params[4]
        ...
        with torch.no_grad():
            ...
            if (iteration in optimization.test_iterations) and (
                ...
                def eval_compressed(viewpoint, gaussians, pipeline, bg):
                    ...
                    res_positions = gaussians.get_xyz - mean3D[node_assignment]

                    compressed_result = eb.compress(
                        res_positions, gaussians, level=len(entropy_lambdas) - 1
                    )
                    ...
                    test_positions = decompressed_result["res_positions"] + mean3D[node_assignment]
